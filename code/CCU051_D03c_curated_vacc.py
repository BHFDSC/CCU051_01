...
display(vacc.where(f.col('PERSON_ID_DEID') == ''))
...
# display(vacc.where(f.col('PERSON_ID_DEID') == '').orderBy('DATE_AND_TIME'))
...
print('---------------------------------------------------------------------------------')
# 20221215 Advised by  to exclude records where VACCINATION_SITUATION_CODE == '1324751000000103' (not null)
...
# display(tmpc3.where(f.col('_rownum') == 19)) # 
...
# display(tmpg)
# display(tmpc6.where(f.col('PERSON_ID') == ''))
