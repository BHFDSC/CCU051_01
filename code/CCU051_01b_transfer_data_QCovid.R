...
for (i in 1:chunks) {
  ...
  tmp <- DBI::dbGetQuery(con, paste0("SELECT * FROM dars_nic_391419_j3w9t_collab.ccu051_out_qcovid WHERE CHUNK='",i,"'"))
  
  df <- rbind(df,tmp)
  
}
# save as table 
data.table::fwrite(df,paste0("ccu051_qcovid",gsub("-","",Sys.Date()),".csv.gz"))
